...
Public Class ExtractionRejet
    ...
    Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        If (Not context.Session("Utilisateur") Is Nothing) Then
            ...
            If (Not ((context.Request.Params("extIdStatutFacture") Is Nothing) Or (context.Request.Params("extIdMotifRejet") Is Nothing))) Then

                Dim id_statut_facture As String = context.Request.Params("extIdStatutFacture").ToString()

                Dim id_rejet As String = context.Request.Params("extIdMotifRejet").ToString()
                ...
                Dim liste_factures As List(Of Facture) = DBFunctions.getFactures(current_user, "", liste_statuts, True, False, True, id_rejet, 0, "").Where(Function(x) x.Rejet.Id <> "5" And x.Rejet.Id <> "6" And x.NomFournisseur <> "EGENCIA").ToList()
                Dim liste_alertes_facture As List(Of AlerteFacture) = DBFunctions.getAlertesFacture("", current_user, False, "")

                Dim ligne As String

                Dim nom_fichier As String = "EXTRACTION_REJET_" & timestramp & ".csv"
