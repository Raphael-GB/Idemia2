...
Public Class JsonFactureEGENCIA
    ...
    Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        If (Not context.Session("Utilisateur") Is Nothing) Then
            ...
            Dim rejet As String = context.Request.QueryString("rejet").ToString()
            ...
            HttpContext.Current.Session("filtre_rejet") = rejet

            'Tri pour la recherche

            Dim lignes As List(Of LigneFactureCorbeille) = getLignesFactures(current_user, rejet, statut).FindAll(Function(x) x.Id.Contains(sSearch) _
                                                                                                                ...
                                                                                                                Or x.MotifRejet.Contains(sSearch) _
                                                                                                                Or x.MotifAlerte.Contains(sSearch) _
                                                                                                                Or x.EmetteurAlerte.Contains(sSearch) _
                                                                                                                Or x.Statut2.Contains(sSearch) _
                                                                                                                Or x.Site.Contains(sSearch))
            ...
            If (iSortDir = "asc") Then
                ...
                ElseIf (iSortCol = 8) Then
                    lignes = lignes.OrderBy(Function(x) x.MotifRejet).ToList()
            ...
            Else
                ...
                ElseIf (iSortCol = 8) Then
                    lignes = lignes.OrderByDescending(Function(x) x.MotifRejet).ToList()
                ElseIf (iSortCol = 9) Then
    ...
    Private Function getLignesFactures(p_user As Utilisateur, p_rejet As String, p_statut As String) As List(Of LigneFactureCorbeille)
        ...
        Try
            ...
            Dim liste_factures As List(Of Facture) = DBFunctions.getFacturesEgencia(p_user, "", liste_statuts, True, False, True, p_rejet, 0, "").ToList()
            ...
            For Each fact As Facture In liste_factures
                ...
                LigneFactureCorbeille.MotifRejet = fact.Rejet.Libelle
                ...
                LigneFactureCorbeille.Site = fact.Site
                If fact.Statut2 = "29" Or fact.Statut2 = "" Then
                    LigneFactureCorbeille.Statut2 = "à traiter"
                Else
                    LigneFactureCorbeille.Statut2 = "traité"
                End If
